...
class CFHMMLayer:
    def __init__(self, M, K, X, minPriorAlpha, maxPriorAlpha, iniBySampling):
        # initially component number is set to K. No shrinkage has occurred
        self.K = K
        self.D = X[0][0].shape[0]
        # a vector for each possible state value
        ...
        # all scale factors are equal to 1/K (originally 1, after normalization)
        self.logStateScaleFactors = np.ones(K) * np.log( 1.0 / K ) 
        self.stateScaleFactorsNorm = K       
    
        self.maxPriorAlpha = maxPriorAlpha
        self.minPriorAlpha = minPriorAlpha
        
        self.priorAlpha = np.zeros(K)
        self.priorAlphaStep = ( maxPriorAlpha - minPriorAlpha ) / ( K - 1 )
        self.priorAlphaSum = 0
        for k in xrange(K):
            self.priorAlpha[k] = minPriorAlpha + self.priorAlphaStep * k
        
        self.priorAlphaSum = np.sum( self.priorAlpha ) 
    ...
    def loadParams(self, i):
        ...
        self.priorAlpha = self.params[i]['priorAlpha']
    ...
    def MStep( self, varDist, m ):
        ...
        self.c_m0k = np.sum( [ qZmn[0] for qZmn in qZm ], axis=0 ) 
        if np.abs( np.sum(self.c_m0k) - self.N ) > 1e-5:
            raise RuntimeError('sum of c_m0k is wrong')
        self.alpha = ( self.c_m0k + self.priorAlpha ) / ( self.priorAlphaSum + self.N )
        
        # normalizers for each row of beta
        self.c_mj_ = np.zeros( self.K )
        for qZmn in qZm:
            self.c_mj_ += np.sum( qZmn[:-1], axis=0 )
        
        # the dimensionality of beta might change due to shrinkage
        # so redeclare its shape
        self.beta = np.zeros( ( self.K, self.K ) )
        # update transition probabilities
        self.c_mjk = np.sum( qZZm[0], axis=0 )
        for qZZmn in qZZm[1:]:
            self.c_mjk += np.sum( qZZmn, axis=0 )
        for j in xrange(self.K):
            if np.abs( np.sum(self.c_mjk[j]) - self.c_mj_[j] ) > 1e-5:
                raise RuntimeError('sum of row c_mjk[j] is wrong')
            # beta shares the same prior param as alpha
            self.beta[j] = ( self.c_mjk[j] + self.priorAlpha ) / ( self.priorAlphaSum + self.c_mj_[j] )
        
        self.betaT = self.beta.T
                    
        protect( self.alpha, self.beta )

        # update component complexities
        self.compComplexities = -0.5 * self.D * ( np.log( self.mass ) - 1 )
        self.compComplexities -= 0.5 * np.log( self.c_m0k + self.priorAlpha )
        for j in xrange(self.K):
            self.compComplexities[j] -= 0.5 * np.sum( np.log( self.c_mjk[j] + self.priorAlpha ) )
        self.compComplexities += 0.5 * np.log( self.priorAlphaSum + self.c_mj_ )
    ...
    def forward_backward( self, N, T, varDist, m ):        
        ...
        for n in xrange(N):
            ...
            for t in xrange(T[n]-1):
                sumj_qZZmnt = np.sum( qZZ[n][t], axis=1 )
                if ( np.abs( sumj_qZZmnt - qZ[n][t] ) > 1e-5 ).any():
                    raise RuntimeError("qZ & qZZ mismatch")
    ...
    def updateMass(self, varDist, m):
        ...
        for qZmn in varDist.qZ[m][1:]:
            self.mass += np.sum(qZmn, axis=0) # sum along t
    ...
    def breakSymmetry(self, active):
        ...
        for k in xrange( 1, len(masses) ):
            ...
            if np.isclose( masses[k-1][1], masses[k][1], rtol=1e-3 ):
                print "Symmetry found between states %d and %d" %( masses[k-1][0], masses[k][0] )
    ...
    def updateStateScaleFactors( self, pruneThres, shrinkRate ):
        self.stateScaleFactorsNorm = 0
        self.logStateScaleFactors = np.zeros( self.K )
        ...
        for k in xrange(self.K):
            if active[k]:
                if self.mass[k] > 0 and self.midMass[k] > 0:
                    self.logStateScaleFactors[k] = -0.5 * shrinkRate * self.D / self.mass[k]
                    self.stateScaleFactorsNorm += np.exp( self.logStateScaleFactors[k] )

        # for debugging
        if self.stateScaleFactorsNorm == 0:
            pass
        
        self.logStateScaleFactors -= np.log( self.stateScaleFactorsNorm )
...
class CollapsedFactorialHiddenMarkovModel:
...
#        self.testSeq = kwargs.get('testSequence', None)
        ...
        self.minPriorAlpha = kwargs.get( 'minPriorAlpha', 0 )
        self.maxPriorAlpha = kwargs.get( 'maxPriorAlpha', 0 )
        ...
        self.layers = [ CFHMMLayer( M, self.Ks[m], X, self.minPriorAlpha, self.maxPriorAlpha, iniBySampling ) for m in xrange(M) ]
    ...
    def updateSigmaW(self, X, varDist):
        # varDist: N, T, M, K
        
        # the hidden states of each layer, layer.K, could be smaller than 
        # the initial K, self.K, after shrinkage 
            ...
            for t in xrange( T[n] ):
                ...
                for m in xrange(self.M):
                    ...
                    if self.iterDoShrink and not self.MLEst:
                        c_m += self.layers[m].logStateScaleFactors
        ...
        for n, Xn in enumerate(X):
            ...
            for t in xrange( T[n] ):
                ...
                for m in xrange(self.M):
                    ...
                    if self.iterDoShrink and not self.MLEst:
                        log_h_ntm += self.layers[m].logStateScaleFactors
        ...
        for n, Xn in enumerate(X):
            ...
            for t in xrange( T[n] ):
                ...
                for m in xrange(self.M):
                    ...
                    K_m = Ks[m]
                    # Eq(93) j != k 
                    J.append( - qZ[m][n][t].reshape(-1,1) * qZ[m][n][t] )
                    # Eq(93) j==k
                    J[-1][ range(K_m), range(K_m) ] = qZ[m][n][t] * ( 1 - qZ[m][n][t] )
                    # J^m W^m'
    ...
    def forward_backward( self, X, varDist ):
    ...
    def calcNegKLDiv( self, X, varDist ):
        ...
        for layer in self.layers:
            ...
            # \Lambda = diag{ W' C^-1 W }, column vector, 1*Km
        ...
        for n in xrange(N):
            for t in xrange( T[n] ):
                ...
                for m,layer in enumerate(self.layers):
                    ...
                    if self.iterDoShrink and not self.MLEst:
                        sum_zlogscale += np.dot( qZ[m][n][t], layer.logStateScaleFactors )
                    WC1_sump_qW[m] = np.dot( sum_qW - qW[m], C1W[m] )
                    summ_qWC1_sump_qW += np.dot( WC1_sump_qW[m], qZ[m][n][t] )
        ...
        for m in xrange(self.M):
            ...
            if self.iterDoShrink:
                for n in xrange( N ):
                    ...
                    sumdelta += T[n] * np.log( layer.stateScaleFactorsNorm )
    ...
    def VStep(self, X, varDist, isExact=True):
        ...
        if self.iterUseZeroOrderApproxH:
            ...
            varDist0 = self.forward_backward( X, varDist0 )
    ...
    def MStep( self, X, varDist ):
        ...
        for m in xrange(self.M):
            ...
            print "%d scale: %s" %( m, str( layer.logStateScaleFactors ) )
            ...
            if pruned:
                ...
                layer.priorAlpha = np.zeros(layer.K)
                for k in xrange(layer.K):
                    layer.priorAlpha[k] = layer.minPriorAlpha + layer.priorAlphaStep * k
                
                layer.priorAlphaSum = np.sum( layer.priorAlpha ) 
